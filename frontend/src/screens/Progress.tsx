  ...
  return (
    <div className="screen fade">
      ...
      {tab === "stats" && <>
        ...
        {p.weekly.length > 0 && <Card><div className="eyebrow" style={{ marginBottom: 12 }}>📅 Регулярность по неделям</div><div className="row" style={{ alignItems: "flex-end", height: 80, gap: 6 }}>{p.weekly.map(w => <div key={w.week} style={{ flex: 1, textAlign: "center" }}><div style={{ height: 12 + w.count * 14, background: "var(--accent)", borderRadius: 6 }} /><div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4 }}>{w.count}</div></div>)}</div></Card>}
        <Card><div className="eyebrow" style={{ marginBottom: 6 }}>📈 Прогресс рабочих весов</div>
      ...
      {tab === "photos" && <>
        ...
        {!photos.length && <Card><div style={{ color: "var(--muted)", textAlign: "center" }}>Пока нет фото. Добавь первое — это будет «Неделя 1».</div></Card>}
